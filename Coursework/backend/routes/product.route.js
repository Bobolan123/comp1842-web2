...
import { isValidImage, deleteImage, validateProductFields } from '../utils/product.utils.js';
    ...
    (req, res) => {
        try {
            ...
            if (!isValidImage(req.file)) {
                return res.status(400).json({ message: 'Only image files (PNG, JPG, JPEG) are allowed!' });
            }
            ...
            console.error(err);
            res.status(500).json({ message: 'Server error, could not upload product.' });
        }
    }
);
...
router.put(
    ...
    updateProduct // Delegate to controller
);
